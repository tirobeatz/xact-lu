...
function PropertiesContentInner({ initialProperties, initialTotal, initialTotalPages }: PropertiesContentProps) {
  ...
  useEffect(() => {
    ...
    // If there are URL filter params, trigger a fetch since initial data is unfiltered
    if (hasParams) {
      setHasUserInteracted(true)
    }
  }, [typeParam, locationParam, minBedsParam, maxPriceParam, propertyTypeParam, minAreaParam])

  // Fetch properties (only after user interaction or URL params)
  ...
  }, [fetchProperties, hasUserInteracted])
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <div className="flex bg-[#F5F3EF] rounded-xl p-1">
                {[
                  ...
                  <button
                    key={type.key}
                    onClick={() => { setListingType(type.key as "ALL" | "SALE" | "RENT"); setPage(1); setHasUserInteracted(true) }}
              ...
              <select
                value={location}
                onChange={(e) => { setLocation(e.target.value); setPage(1); setHasUserInteracted(true) }}
              ...
              <select
                value={propertyType}
                onChange={(e) => { setPropertyType(e.target.value); setPage(1); setHasUserInteracted(true) }}
              ...
              >
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      value={minBeds}
                      onChange={(e) => { setMinBeds(Number(e.target.value)); setPage(1); setHasUserInteracted(true) }}
                  ...
                  <div>
                    ...
                    <select
                      value={minArea}
                      onChange={(e) => { setMinArea(Number(e.target.value)); setPage(1); setHasUserInteracted(true) }}
                      className="w-full h-11 px-4 rounded-xl border border-[#E8E6E3] bg-white text-[#1A1A1A] text-sm outline-none"
                    >
                      <option value={0}>Any</option>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      step="50000"
                      value={maxPrice}
                      onChange={(e) => { setMaxPrice(Number(e.target.value)); setPage(1); setHasUserInteracted(true) }}
          ...
          {viewMode === "grid" && !loading && totalPages > 1 && (
            <div className="text-center mt-12 flex items-center justify-center gap-4">
              <Button
                ...
                disabled={page === 1}
                onClick={() => { setPage(page - 1); setHasUserInteracted(true) }}
              >
                {t.common.previous}
              </Button>
...
export default function PropertiesContent(props: PropertiesContentProps) {
  return (
    <Suspense fallback={<PropertiesLoading />}>
      <PropertiesContentInner {...props} />
    </Suspense>
  )
}
